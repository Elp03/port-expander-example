...
async fn main(_spawner: Spawner) -> ! {
    ...
    loop {
        pins.gp0_0.enable_pull_up(true).expect("failed");
        pins.gp0_1.enable_pull_up(true).expect("failed");
        pins.gp0_2.enable_pull_up(true).expect("failed");
        pins.gp0_3.enable_pull_up(true).expect("failed");
        pins.gp0_4.enable_pull_up(true).expect("failed");
        pins.gp0_5.enable_pull_up(true).expect("failed");
        pins.gp0_6.enable_pull_up(true).expect("failed");
        pins.gp0_7.enable_pull_up(true).expect("failed");
        pins.gp1_0.enable_pull_up(true).expect("failed");
        pins.gp1_1.enable_pull_up(true).expect("failed");
        pins.gp1_2.enable_pull_up(true).expect("failed");
        pins.gp1_3.enable_pull_up(true).expect("failed");
        pins.gp1_4.enable_pull_up(true).expect("failed");
        pins.gp1_5.enable_pull_up(true).expect("failed");

        defmt::info!("Sett all pins high");
        Timer::after(Duration::from_millis(1000)).await;

        pins.gp0_0.enable_pull_up(false).expect("failed");
        pins.gp0_1.enable_pull_up(false).expect("failed");
        pins.gp0_2.enable_pull_up(false).expect("failed");
        pins.gp0_3.enable_pull_up(false).expect("failed");
        pins.gp0_4.enable_pull_up(false).expect("failed");
        pins.gp0_5.enable_pull_up(false).expect("failed");
        pins.gp0_6.enable_pull_up(false).expect("failed");
        pins.gp0_7.enable_pull_up(false).expect("failed");
        pins.gp1_0.enable_pull_up(false).expect("failed");
        pins.gp1_1.enable_pull_up(false).expect("failed");
        pins.gp1_2.enable_pull_up(false).expect("failed");
        pins.gp1_3.enable_pull_up(false).expect("failed");
        pins.gp1_4.enable_pull_up(false).expect("failed");
        pins.gp1_5.enable_pull_up(false).expect("failed");
